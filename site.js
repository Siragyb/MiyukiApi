...
var getTecMundoNews = require('./modulos-api/tech.js')
...
const { stalkInstagram } = require("./modulos-api/insta.js"); // Importa a função do arquivo scraper.JSON
...
app.get('/fundo.mp4', function(req, res){
 ...
 app.use((req, res, next) => {
  // Sobrescreve a função "key.includes" para sempre retornar true
  req.key = { includes: () => true };
  next();
});
...
// Rota pública para dar likes (sem verificação de API key)
app.get('/api/like', (req, res) => {
  ...
  if (!uid || !quantity) {
    return res.status(400).json({ 
      error: "Parâmetros obrigatórios: uid (ID do usuário) e quantity (quantidade de likes)" 
    });
  }

  // Exemplo de resposta (simulando ação)
  res.json({
    success: true,
    message: "Likes adicionados com sucesso!",
    data: {
      uid: uid,          // ID do usuário (exemplo: 218370531)
      quantity: quantity // Quantidade de likes (exemplo: 100)
    }
  });
});
...
app.get("/api/instagram", async (req, res) => {
    ...
    try {
        const dados = await stalkInstagram(username);
        res.json(dados);
    } catch (error) {
...
// Rota para acessar as notícias
app.get("/api/tecmundo", async (req, res) => {
    try {
        const noticias = await getTecMundoNews();
        res.json({ status: "success", noticias });
    } catch (error) {
