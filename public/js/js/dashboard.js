...
function getvisitor(){
 ...
 window.setTimeout("waktu()", 1000);
  ...
  const detik = d.getSeconds();
		...
		document.getElementById("Clock").innerHTML = jam +" : "+ menit +" : "+ detik +" BTR";
	}
...
Sayings = "";
			...
			if (hours >= 18 || hours <= 5){
			Sayings += "<span class='text-light'>Boa Noite 🌚</span>"
			} else if (hours >= 5 && hours <= 12){
			Sayings += "<span class='text-info'>Bom Dia 🌝</span>"
			} else if (hours >= 12 && hours <= 17){
			Sayings += "<span class='text-warning'>Boa Tarde 🌞</span>"
			} else if (hours >= 17 && hours <= 18){
			Sayings += "<span class='text-success'>Boa Tarde 🌜</span>"
			}
