...
const MercadoLivreSearch = async (q) => {
  try {
    const url = `https://lista.mercadolivre.com.br/${removerAcentos(q)}`;
    const response = await axios.get(url, { headers: { ...useragent_1 } });

    if (!response.data) {
      throw new Error('Empty response from Mercado Livre');
    }

    const $ = cheerio.load(response.data);
    const resultado = [];

    $('.andes-card--flat.andes-card--default.andes-card--padding-default').each((i, element) => {
      const product = $(element).find('h2:first').text()?.trim();
      const image = $(element).find('img:first').attr('data-src') || $(element).find('img:first').attr('src');
      const price = $(element).find('.ui-row-price--size-medium > div > span > .price-tag-amount').text()?.replace('R$', 'R$ ').trim();
      const link = $(element).find('a:first').attr('href');

      if (product && image && price && link) {
        resultado.push({ product, image, price, link });
      }
    });
